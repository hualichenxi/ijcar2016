...
create table test_feature_merchant_user_count(
select A.User_id,A.Location_id,A.Merchant_id,IFNULL(B.count,0) as Merchant_User_count
from te_d_new as A left join tmp1 as B on 
A.Merchant_id=B.Merchant_id
);
create index idx_test_Merchant_User on test_feature_merchant_user_count(User_id,Merchant_id,Location_id);

select * from tmp1 order by count desc limit 5;
select * from test_feature_merchant_user_count order by Merchant_User_count desc limit 5;
