...
create table train_feature_buy_location_7days(
select A.User_id,A.Location_id,A.Merchant_id,IFNULL(B.count,0) 
as User_Location_count from tr_d_new as A left join tmp_xx as B on 
A.User_id=B.User_id and A.Location_id=B.Location_id
);

drop table tmp_xx;
drop table tmp_x;
create index idx_feature_train_buy_location_7days on train_feature_buy_location_7days(User_id,Location_id,Merchant_id);
...
create table test_feature_buy_location_7days(
select A.User_id,A.Location_id,A.Merchant_id,IFNULL(B.count,0) 
as User_Location_count from te_d_new as A left join tmp_xx as B on 
A.User_id=B.User_id and A.Location_id=B.Location_id
);
...
create index idx_feature_test_buy_location_7days on test_feature_buy_location_7days(User_id,Location_id,Merchant_id);
-- 
-- 