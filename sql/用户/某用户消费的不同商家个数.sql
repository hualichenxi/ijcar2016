...
create table test_feature_user_merchant_count(
select A.User_id,A.Location_id,A.Merchant_id,IFNULL(B.count,0) as User_Merchant_count
from te_d_new as A left join tmp1 as B on 
A.User_id=B.User_id
);

create index idx_test_User_Merchant on test_feature_user_merchant_count(User_id,Merchant_id,Location_id);
select * from tmp1 order by count desc limit 5;
select * from test_feature_user_merchant_count order by User_Merchant_count desc limit 5;
